...
# BALLOON_CRUMBS_BASEMAP_VERSION. Exits non-zero on the first real failure so it can
# gate a deploy.
set -uo pipefail
...
echo "- style"
style_body="$(curl -sS --max-time 20 "$origin/maps/styles/balloon-crumbs.json" 2>/dev/null)"
style_code="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 20 "$origin/maps/styles/balloon-crumbs.json" 2>/dev/null)"
if [ "$style_code" != "200" ]; then
  ...
  if ! curl -sS -D- -o /dev/null --max-time 20 -H 'Origin: https://example.invalid' \
    "$origin/maps/styles/balloon-crumbs.json" 2>/dev/null \
...
if [ "$tile_code" != "200" ]; then
  ...
  fail "tile is $tile_bytes bytes. An empty 200 means the upstream path is a
        ...
        segment, and that BALLOON_CRUMBS_BASEMAP_VERSION is not empty."
else
