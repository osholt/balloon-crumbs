...
from balloon_crumbs_server.models import Ride, StoredEvent
from balloon_crumbs_server.rate_limit import SlidingWindowRateLimiter
...
def test_health_and_metrics_do_not_require_ride_credentials(client) -> None:
    ...
    assert "balloon_crumbs_sync_requests_total" in metrics.text
...
def test_device_header_must_match_body(client, synchronize) -> None:
    ...
    response = client.post(
        ...
        headers={
            ...
            "x-balloon-crumbs-device": "header-device",
        },
    )
    assert response.status_code == 400
