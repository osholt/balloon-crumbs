...
def classify_push_event(
    ...
    if event_type == "statusMessage":
        ...
        if message_type in {"emergencyStop", "assistance"}:
            return PushMessage(
                ...
                body="Open Hot Pursuit for the authenticated ride details.",
                critical=True,
        ...
        if message_type in {"allPassed", "resolved"}:
            return PushMessage(
                ...
                body="Open Hot Pursuit for the latest group status.",
                critical=False,
                recipient_ids=recipients,
    ...
    if event_type in {"markerStarted", "markerEnded"}:
        return PushMessage(
            ...
            body="Open Hot Pursuit for the current marker status.",
            critical=False,
            recipient_roles=coordinator_roles,
        )
    ...
    if event_type in {"ridePaused", "rideResumed", "rideEnded"}:
        return PushMessage(
            ...
            body="Open Hot Pursuit for the current ride state.",
            critical=False,
            all_members=True,
        )
    return None
