...
# BALLOON_CRUMBS_BUILD_COMMIT, and `--force-recreate caddy` only when the Caddyfile
...
# ---------------------------------------------------------------------------
BALLOON_CRUMBS_BUILD_COMMIT="$(git rev-parse --verify HEAD)"
export BALLOON_CRUMBS_BUILD_COMMIT
...
staging)
  ...
  domain_key="BALLOON_CRUMBS_PREPRODUCTION_DOMAIN"
...
step "Building and starting $target at $BALLOON_CRUMBS_BUILD_COMMIT"
"${compose[@]}" up -d --build
...
if test -n "$caddyfile" && test -n "$previous_commit"; then
  if git diff --quiet "$previous_commit" "$BALLOON_CRUMBS_BUILD_COMMIT" -- "$caddyfile"; then
...
docker run --rm --interactive \
  ...
  --env "SMOKE_EXPECTED_COMMIT=$BALLOON_CRUMBS_BUILD_COMMIT" \
...
printf '%s\n' "$BALLOON_CRUMBS_BUILD_COMMIT" >"$state_file"

step "Deployed $target at $BALLOON_CRUMBS_BUILD_COMMIT"
"${compose[@]}" ps
